...
class ProfileUploadApi {
  final _uploadService = UploadProfileService(Dio(BaseOptions(headers: {'Content-Type': 'application/json', 'Accept': 'application/json'})));
  Future<void> uploadProfileImage(PlatformFile? image) async {
    ...
    try {
      ...
      if (kIsWeb) {
            ...
            String base64String =
                await MimeType.changeToBase64(imagePath);
            requestBody["image"] = {"data": base64String, "mime": mimeType};
          }
    }
      var token = await AuthController().getToken();
      await _uploadService.uploadProfile(requestBody, token!);
    } catch (e) {
      rethrow;
    }
  }
